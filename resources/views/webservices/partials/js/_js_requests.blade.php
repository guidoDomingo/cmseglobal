<script type="text/javascript">
  $(document).ready(function () {
      ...
      function clearWSRequestForm(clear_alert){
        ...
        var add_url = '{{ route("webservicerequests.store") }}';
      ...
      */
      $(document).on('click', '.btn-edit-request', function(e){
          ...
          var id = row.data('id');
          var form = $('#form-ws-request');
          var url = '{{ route("webservicerequests.show","") }}'+'/'+id;
          var type = "";
          var title = ""
          $.ajax({
              type: "get",
              ...
              success: function (object) {
                ...
                if(object.transactional == true){
                  $('#service_provider_product_id').removeAttr('disabled');
                  $('#service_provider_product_id').val(object.service_provider_product_id);
                }
                input_method = '<input type="hidden" name="_method" id="_method" value="PUT" />';
                form.append(input_method);
                var edit_url = '{{ route("webservicerequests.update","") }}'+'/'+id;
                form.attr('action',edit_url);
              }
          });
      });
      $(document).on('change', '#transactional', function(e){
          if(this.checked) {
            $('#service_provider_product_id').removeAttr('disabled');
          }else{
            $('#service_provider_product_id').attr('disabled','disabled');
          }
      });
