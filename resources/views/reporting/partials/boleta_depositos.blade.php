    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box box-default">
                ...
                <!-- /.box-header -->
                <form action="{{route('reporting.boletas_depositos.search')}}" method="GET" id="estadoContable-form">
                                ...
                                <table class="table table-striped" role="grid">
                                    ...
                                    @foreach($transactions as $transaction)
                                        <tr data-id="{{ $transaction->id  }}">
                                                ...
                                                <div class="btn-group">
                                                    ...
                                                    @if( !\Sentinel::getUser()->inRole('mini_terminal') && !\Sentinel::getUser()->inRole('supervisor_miniterminal'))
                                                        @if($transaction->estado)
                                                            <buttom class="btn btn-default btn-xs" title="Detalles de Los Recibos">
                                                                <i class="info_recibos fa fa-info-circle" style="cursor:pointer"></i>
                                                            </buttom>
                                                        @endif
                                                    @endif
                                                    @if(!is_null($transaction->imagen_asociada))
                                            ...
                                            <td>@if(isset($transaction->atm_id)) {{ $transaction->name_atm }} @else -- @endif</td>
                        ...
                        <div class="row">
                            <div class="col-sm-7">
                                <div class="dataTables_paginate paging_simple_numbers">
                                    {!! $transactions->appends(['reservationtime' => $reservationtime, 'atm_id' => $atm_id, 'status_id' => $status_set])->links('paginator') !!}
                                </div>
                            </div>
                        </div>
                    </div>
...
@section('js')
    ...
    <script>
        $(function(){
            ...
            $('#group_id').on('change', function(e){
                var group_id = e.target.value;            
                $.get('{{ url('reports') }}/ddl/atms/' + group_id, function(data_select) {
                    $('#atm_id').empty();
                    $.each(data_select, function(i,item){
            ...
            $('.info_recibos').on('click',function(e){
                e.preventDefault();
                var row = $(this).parents('tr');
                var id = row.data('id');

                $.get('{{ url('reports') }}/info/details_recibos/' + id, function(data) {
                    console.log(data);
                    $(".idTransaccion").html(id);
                    $("#modal-contenido").html(data);
                    $("#status_description").hide();
                    $("#payment_details").hide();
                    //$("#detalles_recibo").show();
                    $('#rechazo_detail').hide();
                    $('#recibo_detail').show();
                    $("#detalles").show();
                    $('#devoluciones').hide();
