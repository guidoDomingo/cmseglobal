...
@section('title')
    Nueva Venta Miniterminal
@endsection
@section('content')
    <section class="content-header">
        <h1>
            Venta
            <small>Creación de Venta de Miniterminales</small>
        </h1>
        <ol class="breadcrumb">
            ...
            <div class="col-md-12">
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        ...
                        {!! Form::open(['route' => 'venta.store' , 'method' => 'POST', 'role' => 'form']) !!}
...
@section('page_scripts')
    ...
    <script>
        ...
        $('#tipo_venta_id').on('select2:select',function(){
            if(this.value == 1 || this.value == 2){
                ...
                if(this.value == 2){
                    $('#num_cuota').val(60);
                    $('#num_cuota').trigger('change.select2');
                    var monto= $('#amount').val() / 60;
                    $("#num_cuota").prop("readonly", true);
                    $('#cant_cuotas').text("Monto en cuotas: " + parseInt(monto) );
                }else{
                    $('#num_cuota').val(null);
                    $('#num_cuota').trigger('change.select2');
                    $("#num_cuota").prop("readonly", false);
                }
            }else{
                $('.num_cuotas').hide();
                var n = 20000000;
        ...
        $('#serial').on('change',function(){
            ...
            if( $('#id_acreedor option:selected').val() == 6){
                var n = 20000000;
                var count = $('#serial option:selected').length;
                var multi= n*count;
                ...
                $('#amount').val(multi);  
                $('#amount').trigger('change.select2');
            }else if($('#id_acreedor option:selected').val() == 7){
                /*var n = 24050000;
                var count = $('#serial option:selected').length;
                var multi= n*count;
        ...
        $('#id_acreedor').on('select2:select',function(){
            if(this.value == 6){

                $('#tipo_venta_id').val('0');
                $('#tipo_venta_id').trigger('change.select2');

                $('#tipo_venta_id').attr('disabled', 'disabled');
                ...
                $('#tipo_venta_id').val(null);
                $('#tipo_venta_id').trigger('change.select2');
                $('#tipo_venta_id').attr('disabled', false);
                
                if($('#tipo_venta_id').val() == 1 ){
                    /*var n = 24050000;
                    var count = $('#serial option:selected').length;
                    var multi= n*count;
...
@endsection
@include('ventas.partials.modal_group')