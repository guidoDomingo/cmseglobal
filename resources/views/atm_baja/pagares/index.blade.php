...
@section('content')
    <section class="content-header">
        <h1>
            PAGARÉ
            <small>Elaboración de documentaciones</small>
        </h1>
        <ol class="breadcrumb">
            ...
            <div class="col-md-12">
                <div class="box box-primary">
                     <div class="box-header with-border">
                        ...
                        @if (Sentinel::hasAccess('atms.group.pagare.add'))
                            {{-- <a href="{{ route('pagares.create',['atm_id' => $atm->id, 'group_id' => $grupo[0]->id]) }}" class="btn-sm btn-primary pull-right active" role="button">Agregar</a> --}}
                            <a href="{{ route('pagares.create',['atm_list' => $atm_ids, 'group_id' => $grupo->id]) }}" class="btn btn-primary mb-2 me-4" role="button">Agregar</a>
                        @endif
                                ...
                                style="width:100%">
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="text-align:center;">Tipo de pagaré</th>
                                        ...
                                        <tbody>
                                            ...
                                            <tr data-id="{{ $item->id }}">
                                                ...
                                                <td style="text-align:center; vertical-align:middle;">{{ $item->firmante }}</td>
                                                <td style="text-align:center; vertical-align:middle;">{{ $item->tasa_interes }} %</td>
                                                <td style="vertical-align:middle;  text-align:center;">{{ number_format($item->monto) }}</td>
                                                <td style="vertical-align:middle; text-align:center;">{{ date('d/m/Y', strtotime($item->vencimiento)) }}</td>
                                                <td style="vertical-align:middle; text-align:center;">
                                                    @if (Sentinel::hasAccess('atms.group.pagare.edit'))
                                                        {{-- <a class="btn btn-success btn-flat btn-row btn-sm" title="Editar" href="{{ route('pagares.edit',['id' => $item->id, 'atm_id' => $atm->id])}}"><i class="fa fa-pencil"></i></a> --}}
                                                        <a class="btn btn-success btn-flat btn-row btn-sm" title="Editar" href="{{ route('pagares.edit',['id' => $item->id])}}"><i class="fa fa-pencil"></i></a>
                                                    @endif
                                                    @if (Sentinel::hasAccess('atms.group.pagare.delete'))
    ...
    {!! Form::open(['route' => ['pagares.destroy',':ROW_ID'], 'method' => 'DELETE', 'id' => 'form-delete']) !!}
    {!! Form::close() !!}
@endsection
...
<script type="text/javascript">            
    ...
    $('#datatable_pagares').DataTable(data_table_config);
</script>
