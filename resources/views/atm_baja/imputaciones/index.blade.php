...
@section('title')
    BAJA | Imputación de deudas
@endsection
@section('content')
    <section class="content-header">
        <h1>
            IMPUTACIÓN DE DEUDAS |
            <small>Gestión Aseguradora</small>
        </h1>
        <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-dashboard"></i> Inicio</a></li>
            <li><a href="#">Gestor de terminales</a></li>
            <li><a href="#">Baja</a></li>
            <li class="active">Imputación de deudas</li>
        </ol>
    </section>
            ...
            <div class="col-md-12">
                <div class="box box-primary">
                    <div class="box-header with-border">
                        ...
                        @if (Sentinel::hasAccess('atms.group.imputacion.edit'))
                        <a href="{{ route('imputaciones.create',['atm_list' => $atm_ids, 'group_id' => $grupo->id]) }}" class="btn btn-primary mb-2 me-4" role="button">Agregar</a>
                        @endif
                    ...
                    <div class="box-body">
                        ...
                        <div class="row">
                            <div class="col-xs-12">
                                <table class="table table-bordered table-hover dataTable" role="grid" id="datatable_imputaciones" style="font-size: 13px">
                                    <thead>
                                        <tr>
                                        ...
                                        <tbody>
                                            @foreach($imputaciones as $item)
                                            <tr data-id="{{ $item->id }}">
                                                <td style="text-align:center; vertical-align:middle;">{{ $item->id }}.</td>
                                                <td style="text-align:center; vertical-align:middle;">{{ $item->numero }}</td>
                                                <td style="text-align:center; vertical-align:middle;">{{ $item->numero_contrato }}</td>
                                                <td style="vertical-align:middle;  text-align:center;">{{ number_format($item->monto) }}</td>
                                                <td style="vertical-align:middle; text-align:center;">{{ date('d/m/Y', strtotime($item->fecha_cobro)) }}</td>
                                                @if ($item->estado == 0)
                                                    <td style="text-align:center; vertical-align:middle;">Cobrado</td>
                                                @else
                                                    <td style="text-align:center; vertical-align:middle;">Pendiente</td>
                                                @endif
                                                <td style="vertical-align:middle; text-align:center;">
                                                    @if (Sentinel::hasAccess('atms.group.imputacion.edit'))
                                                        <a class="btn btn-success btn-flat btn-row btn-sm" title="Editar" href="{{ route('imputaciones.edit',['id' => $item->id])}}"><i class="fa fa-pencil"></i></a>
                                                    @endif
                                                    @if (Sentinel::hasAccess('atms.group.imputacion.delete'))
    ...
    {!! Form::open(['route' => ['imputaciones.destroy',':ROW_ID'], 'method' => 'DELETE', 'id' => 'form-delete']) !!}
    {!! Form::close() !!}
@endsection
...
<script type="text/javascript">            
    ...
    $('#datatable_imputaciones').DataTable(data_table_config);
</script>
